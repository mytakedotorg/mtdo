...
public abstract class Ip {
	public static String get(Request req) {
		return req.require(Ip.class).ipInternal(req);
	}

	protected abstract String ipInternal(Request req);

	public static class Module implements Jooby.Module {
		@Override
		public void configure(Env env, Config conf, Binder binder) throws Throwable {
			if (env.name().equals(HerokuDatabase.ENV)) {
				binder.bind(Ip.class).toInstance(new Ip() {
					@Override
					protected String ipInternal(Request req) {
